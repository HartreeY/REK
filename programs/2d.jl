...
@inbounds function get_2d_migr_params(migr_mode)
    if migr_mode == "4"
        return (1,0)
    elseif migr_mode == "8"
        return (1/2,1/2)
...
@inbounds function calc_2d_migr_dist(deme,pnt_wld_stats,migr_mode,bottleneck,max_migr,refl_walls)
    ...
    if rand() < pnt_wld_stats["migr_rate"] && migr_res < p_lat + p_diag
        if migr_res < p_lat
            dir = sample(MIGR_DIRS_2D_4)
        elseif migr_res < p_lat + p_diag
            dir = sample(MIGR_DIRS_2D_8)
        end
        ...
        if !isnan(bottleneck[1]) && (deme[1]+move_x==bottleneck[1] && deme[2]+move_y!=ceil(pnt_wld_stats["max"][2]/2)) # bottleneck barrier check
            move_x = 0
            move_y = 0
        else
