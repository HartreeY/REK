...
@inbounds function get_3d_migr_params(migr_mode)
    if migr_mode == "6"
        return (1,0)
    elseif migr_mode == "12"
        return (1/2,1/2)
    ...
    elseif migr_mode == "diag1/2"
        return (2/3,1/3)
    end
end

@inbounds function calc_3d_migr_dist(deme,pnt_wld_stats,migr_mode,bottleneck,max_migr,refl_walls)
    p_lat,p_diag = get_2d_migr_params(migr_mode)
    ...
    if rand() < pnt_wld_stats["migr_rate"] && migr_res < p_lat + p_diag
        if migr_res < p_lat
            dir = sample(MIGR_DIRS_3D_6)
        elseif migr_res < p_lat + p_diag
            dir = sample(MIGR_DIRS_3D_12)
        end
        ...
        if !isnan(bottleneck[1]) && (deme[1]+move_x==bottleneck[1] && deme[2]+move_y!=ceil(pnt_wld_stats["max"][2]/2)+move_y!=ceil(pnt_wld_stats["max"][2]/2)) # bottleneck barrier check
            move_x = 0
            move_y = 0
        else
            if deme[1]+move_x > max_migr[1] || deme[1]+move_x < 1 # burn-in area check
                move_x = refl_walls ? -move_x : 0
            end
            if deme[2]+move_y > pnt_wld_stats["max"][2] || deme[2]+move_y < 1
...
@inbounds function build_next_gen(pnt_wld_ms1::Array{Array{Array{Bool}}},pnt_wld_ms2::Array{Array{Array{Bool}}},pnt_wld_stats,pnt_meanf_wld=NaN,pnt_pops_wld=NaN,pnt_muts_AAsel_wld=NaN,pnt_muts_Aasel_wld=NaN,
    ...
    max_migr=NaN,migr_mode=DEF_MIGR_MODE_2D,bottleneck=NaN,refl_walls=false)
    ...
    wld_ms1_next = Array{Array{Array{Bool}}}(undef,pnt_wld_stats["max"]...)
    wld_ms2_next = Array{Array{Array{Bool}}}(undef,pnt_wld_stats["max"]...)
    mean_fitn_next = NaN
    pops_next = NaN
    muts_AAsel_next = NaN
    muts_Aasel_next = NaN
    ...
    wlddim = length(pnt_wld_stats["max"])+1

    if pnt_meanf_wld isa Array{Float32, wlddim}
        meanf_out = true
        mean_fitn_next = Array{Float32}(undef,pnt_wld_stats["max"]...)
        fill!(mean_fitn_next,NaN)
    end
    if pnt_pops_wld isa Array{Float32, wlddim}
        pops_out = true
        pops_next = Array{Float32}(undef,pnt_wld_stats["max"]...)
        fill!(pops_next,NaN)
    end
    if (pnt_muts_AAsel_wld isa Array{Float32, wlddim}) && (pnt_muts_Aasel_wld isa Array{Float32, wlddim}) && (pnt_muts_aasel_wld isa Array{Float32, wlddim})
        sel_out = true
    ...
    if (pnt_muts_AAneu_wld isa Array{Float32, wlddim}) && (pnt_muts_Aaneu_wld isa Array{Float32, wlddim}) && (pnt_muts_aaneu_wld isa Array{Float32, wlddim})
        neu_out = true
    ...
    for deme in next_gen_posits
        ...
        if next_generation_size > 0
            ...
            for _ in 1:next_generation_size
                ...
                move_x, move_y = calc_2d_migr_dist(deme,pnt_wld_stats,migr_mode,bottleneck,max_migr,refl_walls)
...
function rangeexp_axial(n_gens_burnin=DEF_N_GENS_BURNIN,n_gens_exp=DEF_N_GENS_EXP;x_max_burnin=DEF_X_MAX_BURNIN,x_max_exp=DEF_X_MAX_EXP,y_max=DEF_Y_MAX,migr_mode=DEF_MIGR_MODE_2D,
    ...
    if occursin("F", data_to_generate)
        meanf_wld = Array{Float32}(undef, wld_stats["max"]..., 0)
    end
    ...
    @inbounds for g in 1:n_gens_total
        ...
        muts_Aaneu_wld,muts_aaneu_wld;max_migr=(_x_max_used,DEF_Y_MAX),migr_mode=migr_mode,bottleneck=(x_max_burnin*2,0))
        if occursin("F", data_to_generate)
...
@inbounds function build_next_gen_inf(pnt_wld::Array{Array{Array{Float32}}},pnt_wld_stats,pnt_meanf_wld=NaN,pnt_pops_wld=NaN,pnt_muts_delsel_wld=NaN,pnt_muts_bensel_wld=NaN,pnt_muts_delneu_wld=NaN,pnt_muts_benneu_wld=NaN;
    ...
    # Define the habitat (world) and the data arrays in the next generation
    wld_next = Array{Array{Array{Float32}}}(undef,pnt_wld_stats["max"]...)
    mean_fitn_next = NaN
    pops_next = NaN
    ...
    if (pnt_muts_delsel_wld isa Array{Float32, wlddim}) && (pnt_muts_bensel_wld isa Array{Float32, wlddim})
        sel_out = true
    ...
    if (pnt_muts_delneu_wld isa Array{Float32, wlddim}) && (pnt_muts_benneu_wld isa Array{Float32, wlddim})
        neu_out = true
...
function rangeexp_axial_inf(n_gens_burnin=DEF_N_GENS_BURNIN,n_gens_exp=DEF_N_GENS_EXP;x_max_burnin=DEF_X_MAX_BURNIN,x_max_exp=DEF_X_MAX_EXP,y_max=DEF_Y_MAX,migr_mode=DEF_MIGR_MODE_2D,
    ...
    @inbounds for g in 1:n_gens_total
        ...
        wld, mean_fitn_next, pops_next, muts_delsel_next, muts_bensel_next, muts_delneu_next, muts_benneu_next = build_next_gen_inf(wld,wld_stats,meanf_wld,pops_wld,muts_delsel_wld,
            ...
            max_migr=(_x_max_used,DEF_Y_MAX),migr_mode=migr_mode,bottleneck=(x_max_burnin*2,0))
...
function average_front(re,obj="meanf";greaterzero=false,oneside=false,divide=true)
    average_front(re[obj],re["stats"]["n_gens"],re["stats"]["max"]...;greaterzero=greaterzero,oneside=oneside,divide=divide)
end
...
function front_array(re,obj="meanf";oneside=false)
    front_array(re[obj],re["stats"]["n_gens"],re["stats"]["max"]...;oneside=oneside)
end
